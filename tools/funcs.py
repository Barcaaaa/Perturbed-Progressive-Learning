    ...
    def generate_params(self, n_masks, mask_shape, rng=None):
        ...
        if self.prop_by_area:
            ...
            if self.random_aspect_ratio:
                y_props = np.exp(rng.uniform(low=0.0, high=1.0,
            ...
            if self.random_aspect_ratio:
                y_props = rng.uniform(self.prop_range[0], self.prop_range[1],
                                      size=(n_masks, self.n_boxes))
                x_props = rng.uniform(self.prop_range[0], self.prop_range[1],
                                      size=(n_masks, self.n_boxes))
            else:
                x_props = y_props = rng.uniform(self.prop_range[0], self.prop_range[1],
                                                size=(n_masks, self.n_boxes))
            fac = np.sqrt(1.0 / self.n_boxes)
            y_props *= fac
            x_props *= fac
        ...
        return masks  # return cut regions
