...
def _resnet_v1(arch, block, layers, pretrained, **kwargs):
    model = ResNet_v1(block, layers, **kwargs)
    if pretrained:
        state_dict = torch.load("pretrained/%s.pth" % arch)
        model.load_state_dict(state_dict, strict=False)
    return model
...
def resnet50_v1(pretrained=False):
    return _resnet_v1('resnet50_v1', Bottleneck, [3, 4, 6, 3], pretrained,
                      replace_stride_with_dilation=[False, True, True])
